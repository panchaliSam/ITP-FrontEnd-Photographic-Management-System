...
const FeedbackList = () => {
  ...
  return (
    <Container className="mt-4">
      <h1 className="mb-4" style={{ textAlign: 'center' }}><strong>Manage Customer Feedback</strong></h1>
      <br />
      <br />
      <Row className="flex-column">
        {feedbackList.map((feedback, index) => (
          <div key={feedback.FeedbackID} style={{ display: 'flex', marginBottom: '20px' }}>
            ...
            <Col lg={4} md={6} sm={12} className="mb-4">
              <Card style={{ width: '1000px', position: 'relative', marginLeft: '20px', borderRadius: '25px', backgroundColor: '#343434',color:'white' }}>
                <Card.Body>
                  ...
                  <Card.Text><strong>Feedback ID:</strong> {feedback.FeedbackID}</Card.Text> {/* Display Feedback ID */}
                  ...
                  <Button variant="danger" onClick={() => handleDeleteFeedback(feedback.FeedbackID)}>Delete</Button> {/* Delete button */}
                </Card.Body>
              </Card>
            </Col>
          </div>
